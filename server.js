...
wsServer.on('request', function(request) {
  ...
  connectionLocal.on('message', function(message) {
    ...
    if (message.type === 'utf8') {  
      ...
      console.log((new Date()) + ' Received Message from client ' + connectedClientsCount + " : " + jsondata.data);
        
      if(jsondata.data === 'sendMeServerClock'){

          connectToBinaryCom();
          
          console.log((new Date()) + " Server Clock Sent to " + connectedClientsCount); 
...
function sendClientFullname() {
  connectionLocal.sendUTF(JSON.stringify({ type: 'fullname', data: bo_fullname} ));
}
...
// var tokenmaster = '1nQOWj5XA8DiC8K';
